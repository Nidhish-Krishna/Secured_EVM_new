...
'''
   ...
   Works Pending:
	 => Error control mechanisms (Chaincheck, Tamper Detection)
      ...
      Last Updated: September 4 2019, 1613hrs
                                                                                          '''
...
def hexDump(decoded_packet):
	print('\n\n\t--------------------------------------\n\tHex Dump of the Received UDP Packet...')
	print('\t--------------------------------------\n\n 0x01 |\t', end = '')

	col_count=0
	row_count=1
	for item in decoded_packet:
		print(format(int(item,16),'#04x'), end = ' ')
		col_count+=1
		if col_count == 8:
			print(end = '  ')
		elif col_count == 16:
...
### LAYER 3
   ...
   while True:
      ...
      BFkey_A = ''.join(chr(random.randint(65, 123)) for i in range(16))
      BFkey_enc_A = blowfish.Cipher(b'' + BFkey_A.encode())

      mk_enc_BFkey_A = aes_A.encrypt(BFkey_A)
      ...
      if send_buffer == []:
         ...
         for host in dst_hw_ip_list:
            enp, pl = sendPacket(block_to_send, host[0],host[1], BFkey_enc_A, mk_enc_BFkey_A)
            s.send(enp)
...
def recv_msg():
   ...
   while True:
      ...
      BFkey_B = masterkeys[ip_blk].decrypt(received_packet[-16:]).decode()
      BFkey_enc_B = blowfish.Cipher(b'' + BFkey_B.encode())
      ...
      plaintext = b''
      while enc_data != b'':
         plaintext += BFkey_enc_B.decrypt_block(enc_data[:8])
         enc_data = enc_data[8:]
